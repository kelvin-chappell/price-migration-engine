...
| Errors | `sealed trait Failure` (28 subtypes) as ZIO's `E` channel | effectful methods that can fail return `Either[Failure subtype, A]`, not throw - the failure and its context are values. Exceptions are still used internally within an implementation (e.g. to unwind a `try`/`catch` around several fallible steps) but are caught and converted to a `Left` at the trait-method boundary.  Could use Try.toOption for this. (Slices converted before this convention was adopted - `Logging`, `EnvConfig`/`EngineSecrets`, `DynamoDBClient`, `S3`, `CohortStateMachine`, `EmailSender` - still throw at the boundary; `Zuora` is the first to use `Either`, followed by `SalesforceClient`, `DynamoDb`, `CohortTableDdl`, `CohortSpecTable`, and `CohortTable`. New/updated slices should use `Either`, not throw.) |
...
3. **Layer 2 - depend on Layer 1 services:**
   ...
   - `CohortTable` (needs `DynamoDb` + stage config + its config + `Logging`) - `fetch`/`fetchAll` return the
     same lazy `Iterator[Either[...]]` shape as `DynamoDb.query`/`.scan`; `create`/`update` return `Either`,
     preserving the original's `DynamoDbError(reason, Some(_))` vs `Some(...)`-less disambiguation between
     `CohortItemAlreadyPresentFailure` and `CohortCreateFailure`.
