...
- **`cohortTableCreationLambda`** — first handler migrated to Scala 3, its own subproject/jar, template for
  migrating the rest one at a time. Depends on `coreScala3`.
- **`dynamoDb`** / **`stateMachine`** — CloudFormation only, no application code.

Each lambda is an `AWS::Lambda::Function` resource in `lambda/cfn.yaml`; per-lambda deployment only differs by
`Code.S3Key` (jar) and `Handler` (class).
