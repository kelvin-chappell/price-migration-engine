...
- **`lambda`** — remaining handlers not yet migrated (`AmendmentHandler`, `EstimationHandler`, `NotificationHandler`,
  `SalesforcePriceRiseCreationHandler`), sharing one fat jar. Depends on `core`.
- **`cohortTableCreationLambda`**, **`migrationLambda`**, **`subscriptionIdUploadLambda`**,
  **`salesforceNotificationDateUpdateLambda`**, **`salesforceAmendmentUpdateLambda`** — handlers migrated to
